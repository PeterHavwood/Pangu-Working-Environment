...
def run_sessions(input_data_dir, output_data_dir, input_time, forecast_time, ort_sessions):
    # Record the running time
    start_time = time.time()

    # Calculate the number of times of each model works
    work_times_24 = forecast_time//24
    work_times_6 = (forecast_time%24)//6
    work_times_3 = (forecast_time%24%6)//3
    work_times_1 = forecast_time%24%6%3
    ...
    output_time = input_time + datetime.timedelta(hours=forecast_time)
    output_year, output_month, output_day, output_hour = output_time.year, output_time.month, output_time.day, output_time.hour

    output_upper_ncfile_name = 'era5_upper_'+str(output_year).zfill(4)+str(output_month).zfill(2)+str(output_day).zfill(2)+str(output_hour).zfill(2)+'.nc'
    output_surface_ncfile_name = 'era5_surface_'+str(output_year).zfill(4)+str(output_month).zfill(2)+str(output_day).zfill(2)+str(output_hour).zfill(2)+'.nc'
